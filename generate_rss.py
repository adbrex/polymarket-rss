#!/usr/bin/env python3
"""
Polymarket Top Markets RSS Generator
Fetches top 20 trending markets and outputs an RSS feed.
"""
...
FEED_DESC = "Polymarketの本日の取引量上位20マーケット"
...
def build_description(market: dict) -> str:
...
def build_rss(markets: list) -> str:
    ...
    for rank, market in enumerate(markets, 1):
        ...
        SubElement(item, "title").text = f"#{rank} {emoji} {question}（本日 {vol24}）"
        ...
        desc_html = build_description(market)
...
def main():
    ...
    with open(OUTPUT_FILE, "w", encoding="utf-8") as f:
        f.write(rss_content)
    print(f"RSSフィード生成完了: {OUTPUT_FILE}")

    # サマリーをコンソールに出力
    print("\n📊 本日のトレンドTOP20:")
    for i, m in enumerate(markets, 1):
        print(f"  {i:2}. {m.get('question','?')} [{fmt_volume(m.get('volume24hr'))}]")
